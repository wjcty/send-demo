    ...
    try {
        ...
        if (response.data.status === '1') {
            ...
            if (arr.length > 0) {
                ...
                const latestTimestamp = Math.max(...tempArr)
                // 获取当前的时间戳B
                const timestampB = Math.floor(Date.now() / 1000) // 当前时间戳（秒）
                // 计算A到B的时间间隔（秒）
                const timeInterval = timestampB - latestTimestamp

                // 判断是否满24小时（24小时 = 24 * 60 * 60秒）
                const isFull24Hours = timeInterval < 24 * 60 * 60
                return isFull24Hours
            }
